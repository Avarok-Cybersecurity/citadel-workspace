use citadel_internal_service_types::{InternalServiceRequest::PeerConnect, InternalServiceResponse};
...
pub async fn peer_connect(
    ...
    match response {
        InternalServiceResponse::PeerConnectSuccess(_) => {
            Ok(PeerConnectResponseTS{success: true, message: None})
        },
        InternalServiceResponse::PeerConnectFailure(r) => {
            println!("Peer connect failed: {}", r.message);
            Ok(PeerConnectResponseTS{success: false, message: Some(r.message)})
        },
        other => {
            panic!(
