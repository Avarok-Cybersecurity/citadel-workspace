...
fn main() {
    ...
    let wasm_client_dir = citadel_internal_service_root.join("citadel-internal-service-wasm-client");
    ...
    if typescript_client_dir.exists() {
        println!("cargo:warning=Copying WASM files to citadel-internal-service/typescript-client...");
    ...
    if client_ts_pkg_dir.parent().map(|p| p.exists()).unwrap_or(false) {
...
fn copy_wasm_files(src: &Path, dst: &Path) -> std::io::Result<()> {
  ...
  "sideEffects": [
    ...
    if dst.ends_with("wasm-client-ts") || dst.ends_with("typescript-client") {
        ...
        println!("cargo:warning=Created proper package.json in {}", dst.display());
    }
...
fn generate_typescript_types(citadel_internal_service_root: &Path, workspace_root: &Path) {
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&generate_script, perms)
    ...
    let src_types_dir = citadel_internal_service_root.join("citadel-internal-service-types/bindings");
