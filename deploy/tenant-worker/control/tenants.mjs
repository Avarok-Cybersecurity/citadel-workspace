...
import { digestsEqual, randomHex, seal, sha256Hex, unseal } from "./secrets.mjs";
import { CREATE_ACTION, verifyTurnstile } from "./turnstile.mjs";
import { priceIds, stripe } from "./stripe.mjs";
...
const CREATE_FIELDS = new Set(["slug", "display_name", "tier", "interval", "seats", "storage_blocks", "turnstile_token"]);
...
export function readCreate(body) {
  ...
  if (!isPaid(body.tier)) {
    ...
    return { plan, token: body.turnstile_token };
  }
  ...
  return { plan: { ...plan, interval: body.interval, seats: body.seats, storage_blocks: blocks }, token: body.turnstile_token };
}
...
export async function createTenant(io, cfg, body, ip) {
  ...
  if (!(await io.store.reserve(row, now))) return refuse("slug-taken", "that address is taken", 409);
  ...
  try {
    ...
    await io.store.attachCheckout(plan.slug, row.tenant_id, await sha256Hex(checkout.id), await seal(checkout.id, claim, row.tenant_id));
    return json({ slug: plan.slug, status: "pending", workspace_host: hostOf(cfg, plan.slug), checkout_url: checkout.url }, 201);
  } catch (e) {
...
export async function tenantStatus(io, cfg, slug, sessionId) {
  ...
  const claim = await unseal(sessionId, held.claim_sealed, held.tenant_id);
