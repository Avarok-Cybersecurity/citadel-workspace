...
//!
//! `citadel-workspaces/src/lib/profile-privacy.ts` mirrors three metadata keys
...
use citadel_workspace_server_kernel::kernel::profile_update::{
    ACCEPTS_REQUESTS_FROM_STRANGERS_KEY, SHOWS_ONLINE_STATUS_KEY, SHOW_PROFILE_TO_STRANGERS_KEY,
};
...
const TS_PRESENCE_KEY: &str = "shows_online_status";
...
fn profile_privacy_mirror_matches_the_server() {
    ...
    assert_eq!(
        SHOWS_ONLINE_STATUS_KEY, TS_PRESENCE_KEY,
        "update profile-privacy.ts"
    );
    assert_eq!(
