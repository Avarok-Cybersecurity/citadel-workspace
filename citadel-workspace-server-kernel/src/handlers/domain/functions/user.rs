pub mod user_ops {
    ...
    pub(crate) fn add_user_to_domain_inner(
        ...
        let mut user_to_add = tx.get_user(target_user_id).cloned().ok_or_else(|| {
    ...
    pub(crate) fn remove_user_from_domain_inner(
        ...
        let mut user_being_removed = tx.get_user(target_user_id).cloned().ok_or_else(|| {
            NetworkError::msg(format!("User to remove {} not found", target_user_id))
        })?;

        println!(
            "[DEBUG] User permissions BEFORE clearing: {:?}",
            user_being_removed.permissions
        );
        user_being_removed.permissions.remove(domain_id);
        println!(
            "[DEBUG] User permissions AFTER clearing: {:?}",
            user_being_removed.permissions
        );

        // Update the user through the transaction to persist the permission changes
        tx.update_user(target_user_id, user_being_removed)?;

        // Verify the update worked by reading the user back
        let updated_user = tx.get_user(target_user_id).ok_or_else(|| {
            NetworkError::msg(format!("User {} not found after update", target_user_id))
        })?;
        println!(
            "[DEBUG] User permissions AFTER update in transaction: {:?}",
            updated_user.permissions
        );

        info!(
            "Permissions for user {} in domain {} cleared and persisted",
            target_user_id, domain_id
        );
