...
use crate::structs::{User, UserRole, Domain, Permission};
...
impl<R: Ratchet> WorkspaceServerKernel<R> {
    pub fn add_member(
        ...
        if !self.is_admin(admin_id) {
            return Err(NetworkError::msg("Permission denied: Only admins can add members"));
        }
    ...
    pub fn remove_member(
        ...
        // Check if admin has permission
        if !self.is_admin(admin_id) {
            return Err(NetworkError::msg("Permission denied: Only admins can remove members"));
        }
    ...
    pub fn update_member_role(
        ...
        // Check if admin has permission
        if !self.is_admin(admin_id) {
            return Err(NetworkError::msg("Permission denied: Only admins can update member roles"));
        }
            ...
            Some(user) => {
                ...
                match operation {
                    crate::commands::PermissionEndowOperation::Add => {
                        ...
                        let perms = user.permissions.entry(domain_id.to_string())
    ...
    fn grant_domain_permissions(&self, user_id: &str, domain_id: &str) -> Result<(), NetworkError> {
        ...
        if let Some(user) = users.get_mut(user_id) {
            ...
            match domains.get(domain_id) {
                Some(Domain::Office { .. }) => {
                    let permissions = user.permissions.entry(domain_id.to_string())
                        .or_insert_with(HashSet::new);
    ...
    fn revoke_domain_permissions(&self, user_id: &str, domain_id: &str) -> Result<(), NetworkError> {
        let mut users = self.users.write().unwrap();
