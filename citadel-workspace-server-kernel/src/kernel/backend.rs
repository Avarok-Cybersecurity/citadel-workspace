use citadel_sdk::prelude::{
    BackendHandler, NetworkError, NodeRemote, ProtocolRemoteExt, Ratchet, SymmetricIdentifierHandle,
};
...
pub async fn generate_remote<R: Ratchet>(
    ...
    cid: u64,
    peer_cid: Option<u64>,
) -> Result<SymmetricIdentifierHandle<R>, NetworkError> {
    node_remote
        .propose_target(cid, peer_cid.unwrap_or(0))
        .await
        .map(|handle| handle.into_owned())
        .map_err(|e| NetworkError::msg(format!("Failed to generate remote handle: {}", e)))
}
...
pub async fn set<R: Ratchet>(
    ...
    handle.set(key.as_ref(), value.into()).await
}
...
pub async fn get<R: Ratchet>(
    ...
    handle.get(key.as_ref()).await
}

pub async fn get_string<R: Ratchet>(
    handle: &SymmetricIdentifierHandle<R>,
    key: impl AsRef<str>,
    ...
    handle
        .get(key.as_ref())
        .await
        .map(|v| v.map(|v| String::from_utf8_lossy(&v).to_string()))
}
...
impl<R: Ratchet, T: Serialize + for<'de> Deserialize<'de> + Clone + Send + Sync>
    BackendStore<R, T>
{
...
impl<R: Ratchet, T: Serialize + for<'de> Deserialize<'de> + Clone + Send + Sync> Default
    for BackendStore<R, T>
{
    fn default() -> Self {
