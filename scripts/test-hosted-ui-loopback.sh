...
[ -x "$AGENT_BIN" ] || { echo "::error::no agent binary at $AGENT_BIN (cargo build -p citadel-workspace-internal-service)"; exit 1; }
...
WORK="$(mktemp -d)"; CTR="hosted-ui-loopback-$$"; AGENT_PID=""
cleanup() { [ -n "$AGENT_PID" ] && kill "$AGENT_PID" 2>/dev/null || true; docker rm -f "$CTR" >/dev/null 2>&1 || true; rm -rf "$WORK"; }
trap cleanup EXIT
...
[ "$agent" = "up" ] || { echo "::error::the agent never listened on $AGENT_PORT"; tail -5 "$WORK/agent.log"; exit 1; }
