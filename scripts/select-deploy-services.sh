...
#
# deploy.sh needs it to reconcile the services a slimmed-down deployment DROPPED, which by
# definition are the ones the compose file no longer mentions - so they cannot be derived from
# the file, and the set has to come from somewhere. It comes from here, for the same reason the
# selection does: two hand-maintained lists of "the services we deploy" drift, and the direction
# they drift in is a service nobody pulls, verifies, restarts, or cleans up.
