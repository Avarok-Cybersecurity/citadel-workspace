...
function Chat() {
  ...
  const handleSend = () => {
    ...
    setMessages(messages => [...messages, { user: currentUser, message: messageInput, timestamp: new Date() }]);
    ...
    <div className="">
      <div>
        {messages.map((message, index) => (
          <p key={index}><b>{message.user}:</b> {message.message} <i>{message.timestamp.toLocaleTimeString()}</i></p>
        ))}
      </div>
      <form>
        <div className="flex">
